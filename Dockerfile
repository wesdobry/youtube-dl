...
RUN set -x && \
    ARCH=`uname -m` && \
    if [ "$ARCH" = "x86_64" ]; then \
        wget -q $(wget -q https://api.github.com/repos/yt-dlp/FFmpeg-Builds/releases/latest -O - | grep -ioE 'https://github.com/yt-dlp/FFmpeg-Builds/releases/download/.*?-linux64-gpl.tar.xz') -O - | tar -xJ -C /tmp/ && \
